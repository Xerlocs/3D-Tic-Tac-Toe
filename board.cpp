...
int Board::evaluate(const int depth)
{
    if (hasXWon())
     //std::cout << "X won, evaluation at depth " << depth << std::endl;
        return (65 - depth);  // Si 'X' ha ganado, es bueno para el maximizador (X)
    if (hasOWon())
     //std::cout << "O won, evaluation at depth " << depth << std::endl;
        return (depth - 65); // Si 'O' ha ganado, es bueno para el minimizador (O)
    if (isFull())
